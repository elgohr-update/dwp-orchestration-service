...
resource "aws_security_group_rule" "egress_host_to_livy_proxy" {
  description              = "Allow host TCP to Livy Proxy"
  protocol                 = "tcp"
  from_port                = 443
  to_port                  = 443
  security_group_id        = aws_security_group.user_host.id
  source_security_group_id = var.livy_proxy_sg_id
  type                     = "egress"
}

resource "aws_security_group_rule" "ingress_livy_proxy_from_host" {
  description              = "Allow host TCP to Livy Proxy"
  protocol                 = "tcp"
  from_port                = 443
  to_port                  = 443
  security_group_id        = var.livy_proxy_sg_id
  source_security_group_id = aws_security_group.user_host.id
  type                     = "ingress"
}
